...
export default function NotificationsBell() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await api.get("/notifications", { params: { page: 1, pageSize: 20 } });
                  ...
                  <li key={n.id}>
                    <button
                      ...
                      className={`block w-full px-4 py-3 text-left text-sm hover:bg-slate-50 ${
                        n.read ? "text-slate-600" : "bg-blue-50/60 font-medium text-slate-900"
                      }`}
                    >
                      <p>{n.message}</p>
                      <p className="mt-1 text-xs font-normal text-slate-400">{formatDate(n.createdAt)}</p>
                    </button>
                  </li>
                ))}
              </ul>
