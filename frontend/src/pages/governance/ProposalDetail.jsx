...
export default function ProposalDetail() {
  ...
  const myVote = proposal?.approvals?.find(
    (a) => a.approverId === user?.id
  );

  const myObjection = proposal?.objections?.find(
    (o) => o.objectorId === user?.id
  );
  ...
  async function handleApprove() {
    ...
    try {
      ...
      if (err.message !== "step-up cancelled") {
        setActionError(
          apiErrorMessage(err, "Could not approve this proposal.")
        );
      }
    } finally {
      setBusyAction("");
  ...
  async function handleObject() {
    const reason =
      window.prompt("Reason for objecting (optional):") || undefined;
    ...
    try {
      ...
      setActionError(
        apiErrorMessage(err, "Could not object to this proposal.")
      );
    } finally {
      setBusyAction("");
    }
  ...
  async function handleExecute() {
    if (
      !window.confirm(
        "Execute this proposal now? This applies the change immediately."
      )
    ) {
      return;
    }
    ...
    try {
      const stepUpToken = await requestStepUp(
        "execute this governance proposal"
      );
      ...
      if (err.message !== "step-up cancelled") {
        setActionError(
          apiErrorMessage(err, "Could not execute this proposal.")
        );
      }
    } finally {
      setBusyAction("");
      ...
      <div className="grid min-w-0 grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="min-w-0 space-y-6 lg:col-span-2">
          <Card className="p-4 sm:p-5">
            ...
            <pre className="max-w-full overflow-x-auto whitespace-pre-wrap break-all rounded-md bg-slate-50 p-3 text-xs text-slate-700">
              ...
              <ul className="space-y-3">
                {proposal.objections.map((o) => (
                  ...
                  >
                    ...
                    {o.reason && (
                      <p className="mt-1 wrap-break-words text-slate-700">
                        {o.reason}
                      </p>
                    )}
                  </li>
        ...
        <div className="min-w-0 space-y-6">
          <Card className="p-4 sm:p-5">
            ...
            <dl className="space-y-2 text-sm">
              ...
              {proposal.executesAfter && (
                <Row label="Executable after">
                  {formatDate(proposal.executesAfter)}
                </Row>
              )}
          ...
          {isPending && (
            <Can permission="governance:approve">
              <Card className="p-4 sm:p-5">
                ...
                <p className="mb-3 wrap-break-words text-xs leading-5 text-slate-500">
                  Eligibility (pool membership, cross-tier co-sign, or
                  auditor quorum) is verified by the server — this panel
                  shows the actions available to your role, not a
                  guarantee you're eligible for this specific proposal.
                </p>

                {myVote ? (
                  <p className="wrap-break-words text-sm text-emerald-700">
                    ✓ You approved this as{" "}
                    {titleCase(myVote.approverPoolRole)}.
                  </p>
                ) : myObjection ? (
                  <p className="text-sm text-red-700">
                    You objected to this proposal.
                  </p>
                ) : (
              ...
              <Card className="p-4 sm:p-5">
                ...
                <p className="mb-3 wrap-break-words text-xs leading-5 text-slate-500">
                  Re-checks quorum from the actual approval records at
                  the moment you execute — it will fail if quorum isn't
                  met yet.
                </p>
...
function Row({ label, children }) {
  return (
    <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
      ...
      <dd className="min-w-0 wrap-break-words text-left text-slate-900 sm:text-right">
        {children}
      </dd>
    </div>
  );
