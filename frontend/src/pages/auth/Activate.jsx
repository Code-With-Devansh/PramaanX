  ...
  async function submit(e) {
    ...
    try {
      ...
      setError(
        apiErrorMessage(
          err,
          "Could not activate this account. The link may be invalid or expired."
        )
      );
    } finally {
      setBusy(false);
  ...
  if (done) {
    return (
      <div className="flex min-h-screen w-full items-center justify-center overflow-x-hidden bg-slate-100 px-4 py-6 sm:px-6 sm:py-8">
        <Card className="w-full max-w-sm min-w-0 p-5 text-center sm:p-6">
          <h1 className="text-lg font-semibold text-slate-900">
            Account activated
          </h1>

          <p className="mt-2 text-sm leading-5 text-slate-600">
            Your password has been set. Sign in to continue — you'll be asked
            to enroll MFA.
          </p>
      ...
      <div className="w-full max-w-sm min-w-0">
        <div className="mb-6 text-center sm:mb-7">
          ...
          <p className="mt-1 text-sm leading-5 text-slate-500">
            Set a password to finish setting up your access.
          </p>
        </div>
